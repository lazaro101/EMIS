$(document).ready(function(){
  ...
  $('.form.main').submit(function(e){
    ...
    if (!$('.form.main').form('is valid')) {
      ...
      $('.cont').scrollTop(1300);
      e.preventDefault();
    // } else if ($('.dropdown.status').dropdown('get value') == "Booked" ) {
    //   if ($('input[name=amtpaid]').val() < parseFloat($('.total.pay').text().replace(/[^0-9-.]/g, '')) / 2 ) {
    //     swal('Error booking event!','Payment must be half paid.','error');
    //     $('.cont').scrollTop(0);
    //     e.preventDefault();
    //   }
    // } else if ($('.dropdown.status').dropdown('get value') == "Completed" ) {
    //   if ($('input[name=amtpaid]').val() != parseFloat($('.total.pay').text().replace(/[^0-9-.]/g, ''))  ) {
    //     swal('Error completing event!','Payment must fully paid.','error');
    //     $('.cont').scrollTop(0);
    //     e.preventDefault();
    //   }
    } else {
  ...
  $('.segment.content .ui.dropdown').dropdown();
  $('.ui.dropdown.pkg').dropdown();
  ...
  $.fn.form.settings.rules.checkNumber = function(value,checkNumber) { 
    if (checkNumber == 1) {
      if ($('.content form input[name=cpnum1]').val() == value) {
        return false;
      }
    } else {
      if ($('.content form input[name=telnum1]').val() == value) {
        return false;
      }
    }
    return true;
  ...
  $('.back.button').click(function(){
    window.open('/Admin/Transaction/Event-Reservation','_self');
  });
  $('.export.button').click(function(){
    window.open('/Admin/Transaction/Event-Reservation?eventdetails=eventpdf&id='+$('input[name=eventrsrvtnid]').val(),'_blank');
  });
  $('.delete.button').click(function(){
    window.open('/deleteReservation?id='+$('input[name=eventrsrvtnid]').val(),'_self');
  });
  $('.cancel.button').click(function(){
    window.open('/cancelReservation?id='+$('input[name=eventrsrvtnid]').val(),'_self');
  });
  $('#addmenu table').DataTable();
  // $('#addstaff table').DataTable();
});
